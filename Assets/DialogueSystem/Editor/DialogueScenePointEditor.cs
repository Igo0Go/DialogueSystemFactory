...
public class DialogueScenePointEditor : Editor
{
    ...
    private void DrawUIMenu()
    {
        ...
        GUILayout.BeginHorizontal();
        GUILayout.Label("Панель-подложка субтитров:");

        EditorGUI.BeginChangeCheck();
        point.subsPanel = (GameObject)EditorGUILayout.ObjectField(point.subsPanel, typeof(GameObject),
           allowSceneObjects: true, GUILayout.MaxWidth(120));
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(point, "Изменена панель-подложка субтитров");
            EditorUtility.SetDirty(point);
        }
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("UI-текст для субтитров:");
        EditorGUI.BeginChangeCheck();
        point.subsText = (Text)EditorGUILayout.ObjectField(point.subsText, typeof(Text),
           allowSceneObjects: true, GUILayout.MaxWidth(120));
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(point, "Изменён UI-текст субтитров");
            EditorUtility.SetDirty(point);
        }
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Панель-подложка текста события:");
        EditorGUI.BeginChangeCheck();
        point.messagePanel = (GameObject)EditorGUILayout.ObjectField(point.messagePanel, typeof(GameObject),
           allowSceneObjects: true, GUILayout.MaxWidth(120));
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(point, "Изменена панель-подложка текста события");
            EditorUtility.SetDirty(point);
        }
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("UI-текст события:");
        EditorGUI.BeginChangeCheck();
        point.messageText = (Text)EditorGUILayout.ObjectField(point.messageText, typeof(Text),
          allowSceneObjects: true, GUILayout.MaxWidth(120));
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(point, "Изменён UI-текст события");
            EditorUtility.SetDirty(point);
        }
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Панель пропуска реплики:");
        EditorGUI.BeginChangeCheck();
        point.skipTip = (GameObject)EditorGUILayout.ObjectField(point.skipTip, typeof(GameObject),
           allowSceneObjects: true, GUILayout.MaxWidth(120));
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(point, "Изменена панель пропуска реплики");
            EditorUtility.SetDirty(point);
        }
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Панель-подложка текста-подсказки:");
        EditorGUI.BeginChangeCheck();
        point.tipPanel = (GameObject)EditorGUILayout.ObjectField(point.tipPanel, typeof(GameObject),
           allowSceneObjects: true, GUILayout.MaxWidth(120));
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(point, "Изменена панель-подложка текста-подсказки");
            EditorUtility.SetDirty(point);
        }
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("UI-текст подсказки:");
        EditorGUI.BeginChangeCheck();
        point.tipText = (Text)EditorGUILayout.ObjectField(point.tipText, typeof(Text),
          allowSceneObjects: true, GUILayout.MaxWidth(120));
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(point, "Изменён UI-текст подсказка");
            EditorUtility.SetDirty(point);
        }
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Текст подсказки:");
        EditorGUI.BeginChangeCheck();
        point.tipString = EditorGUILayout.TextField(point.tipString, GUILayout.MaxWidth(120));
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(point, "Изменён текст подсказки");
            EditorUtility.SetDirty(point);
        }
        GUILayout.EndHorizontal();

        GUILayout.BeginVertical();
        GUILayout.Label("Варианты ответа:");
        if (GUILayout.Button("+", GUILayout.MinWidth(20), GUILayout.MaxWidth(20)))
        {
            point.answers.Add(new AnswerUI());
            Undo.RecordObject(point, "Изменены варианты ответов");
            EditorUtility.SetDirty(point);
        }
        answersScroll = GUILayout.BeginScrollView(answersScroll, GUILayout.MaxHeight(point.answers.Count > 3 ? 70 : point.answers.Count * 25));
        if (point.answers.Count > 0)
        {
            EditorGUI.BeginChangeCheck();
            for (int i = 0; i < point.answers.Count; i++)
            {
                GUILayout.BeginHorizontal();
                GUILayout.BeginHorizontal();
                GUILayout.Label("Кнопка выбора:");
                point.answers[i].variantButton = (GameObject)EditorGUILayout.ObjectField(point.answers[i].variantButton, typeof(GameObject),
                    allowSceneObjects: true);
                GUILayout.EndHorizontal();

                GUILayout.BeginHorizontal();
                GUILayout.Label("UI-текст выбора:");
                point.answers[i].variantText = (Text)EditorGUILayout.ObjectField(point.answers[i].variantText, typeof(Text),
                    allowSceneObjects: true);
                GUILayout.EndHorizontal();

                if (GUILayout.Button("X", GUILayout.MaxWidth(20), GUILayout.MinWidth(20)))
                {
                    point.answers.Remove(point.answers[i]);
                }
                GUILayout.Space(20);
                GUILayout.EndHorizontal();
            }
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(point, "Изменены элементы ответов для " + name);
                EditorUtility.SetDirty(point);
            }
        }
